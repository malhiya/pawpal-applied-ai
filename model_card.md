...
The biggest limitation is how rigid the keyword retrieval still is. If a user phrases something in a way I did not anticipate, the system silently falls back to defaults with no warning, and that kind of failure is hard to notice unless you are actively looking for it. Adding Groq helped with ambiguous phrasing, but only after retrieval already ran. If the retriever does not find anything useful, the LLM does not have enough context to do much better either. I also ran into cases where the LLM's response did not come back in the format the code expected, which caused it to fall back to the keyword classifier without the user knowing. Working with Claude Code was helpful when building out the Groq integration. The suggestion I found most useful was to only pass the relevant lines from the knowledge base to the LLM instead of the whole file, which keeps the prompt focused. That is how the pipeline works now and it made a noticeable difference. On the other side, Claude suggested surfacing a confidence score in the UI for each classification, which sounded useful but would have added a lot of complexity for something the fallback already handles quietly. I decided to skip it. What surprised me most during testing was how many times the unit tests passed but the app still had edge cases I only found by actually using it. I believe there are still more that will come up the more it gets used.
